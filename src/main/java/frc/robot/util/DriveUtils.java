...
public class DriveUtils {
    ...
    public static double deadbandExponential(double spd, int exp, double deadband) {
        return Math.abs(spd) < deadband ? 0
                : DriveUtils.signedPow(spd - deadband * Math.signum(spd), exp) / signedPow(1 - deadband, exp);
    }
    ...
    public static DoubleSupplier cheesyTurn(DoubleSupplier speed, DoubleSupplier turnRate, double minTurn,
            double maxTurn) {
    ...
    public static Pose2d redBlueTransform(Pose2d pose) {
        return pose;// .relativeTo(new Pose2d(16.54, 8.22, Rotation2d.fromDegrees(180))); // 8.22
    }
    ...
    public static Pose2d relativeToReverse(Pose2d thisPose, Pose2d otherPose) {
        ...
        // It applies a positive rotation into the new frame and then a translation in
        // that frame
        Translation2d translation = (thisPose.getTranslation()

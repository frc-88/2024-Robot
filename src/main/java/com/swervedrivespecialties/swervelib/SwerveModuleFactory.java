...
public class SwerveModuleFactory<DriveConfiguration, SteerConfiguration> {
    ...
    private static class ModuleImplementation implements SwerveModule {
        ...
        public boolean areAllCANDevicesPresent() {
            return driveController.getMotor().getMotorVoltage().getValueAsDouble() > 6 && steerController.getMotor().getMotorVoltage().getValueAsDouble() > 6 && steerController.getEncoder().isPresent();
        }
    }
}